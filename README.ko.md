...
- 한 번의 클릭으로 진단 보고서 생성. 로컬 DNS 서버(실행 상태, 수신 포트, 관리 도메인 수, 마지막 시작 실패 원인, 상위 DNS, 쿼리 통계), 현재 매핑 내용, `127.0.0.1`에 대한 라이브 자가 테스트, 네이티브 VPN 터널 보고서를 망라
- 터널 전달 포트와 수신 포트의 일관성 검사 — 모바일에서 「모든 앱이 DNS 불통이지만 IP 직접 접근은 정상」이 되는 가장 큰 원인
- 보고서 전체 복사와 로그 지우기 지원. 보고서는 필요 시 생성됨(오래된 상태가 남지 않음)
...
- 데이터 저장 폴더 변경(이전 가능)
- 시스템 hosts 읽기 전용 패널(데스크톱): 접기/펼치기 상태와 드래그한 높이가 저장되어 시작 시 복원됨
...
| `npm run build:desktop:debug` | 데스크톱 디버그 빌드(서명 없음, 디버그 가능, 코드 최적화 없음) |
...
- Windows에서는 앱 시작 시 관리자 권한으로 자동 상승합니다(시작 시 UAC 창 표시). 시스템 hosts에 쓰기 위한 것으로 정상 동작입니다
- 데스크톱에서 메인 창을 닫으면 종료되지 않고 시스템 트레이로 숨겨집니다. 완전히 종료하려면 트레이 메뉴(메인 창 표시 / 종료) 또는 설정 → 종료를 사용하세요. 앱을 다시 실행해도 기존 창이 포커스됩니다(싱글 인스턴스)
