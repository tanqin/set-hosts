...
- ワンクリックで診断レポートを生成。ローカル DNS サーバー（実行状態、待ち受けポート、管理ドメイン数、前回起動失敗理由、上流 DNS、クエリ統計）、現在のマッピング内容、`127.0.0.1` へのライブ自己テスト、ネイティブ VPN トンネルレポートを網羅
- トンネル転送ポートと待ち受けポートの一致性チェック——モバイルで「全アプリが DNS 不通だが IP 直アクセスは正常」になる一番の原因
- レポート全体のコピーとログクリアに対応。レポートは必要時に生成される（古い状態は残らない）
...
- データ保存ディレクトリの変更（移行可能）
- システム hosts 読み取り専用パネル（デスクトップ）：折りたたみ状態とドラッグした高さが保持され、起動時に復元
...
| `npm run build:desktop:debug` | デスクトップデバッグビルド（未署名、デバッグ可能、コード最適化なし） |
...
- Windows ではアプリ起動時に管理者権限へ自動昇格します（起動時に UAC ダイアログが表示されます）。システム hosts へ書き込むためで、正常な動作です
- デスクトップでメインウィンドウを閉じると終了せずシステムトレイに隠れます。完全に終了するにはトレイメニュー（メインウィンドウを表示 / 終了）か設定 → 終了を使ってください。アプリを再起動しても既存ウィンドウがフォーカスされます（シングルインスタンス）
